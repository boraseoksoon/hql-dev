...
export async function transpileCLI(
  ...
  await writeOutput(code, outPath, logger);
  logger.log(`Entry processed and output written to ${outPath}`);
  ...
  stop();
  logger.log(`Successfully bundled output to ${outPath}`);
  return outPath;
}
...
export async function bundleCode(
  ...
  if (isHql) {
    ...
    const ast = parse(userSource);
    const originalDir = dirname(resolvedInputPath);
    const transformed = await transformAST(ast, originalDir, { bundle: true, verbose: options.verbose });
    code = rebaseImports(transformed, originalDir);
  } else {
    logger.log(`Using JavaScript entry file: ${resolvedInputPath}`);
    code = await readTextFile(resolvedInputPath);
  ...
  // IMPORTANT: use the directory of the input as the base.
  const targetDir = new URL('.', `file://${dirname(resolvedInputPath).replace(/\\/g, "/")}/`).href;
  code = code.replace(
    /(from\s+['"])(\.{1,2}\/[^'"]+)(['"])/g,
    (_, prefix, rel, suffix) => {
      try {
  ...
  const finalCode = await inlineHqlImports(code, targetDir, logger);
  logger.log(`Bundling complete.`);
  return finalCode;
}

/**
 * Bundle the code using esbuild (with our HQL and external npm plugins) and write it to the specified output.
 */
