...
function createExpressionStatement(
  ...
  return perform(
    ...
    CodeGenError,
    ["expression statement creation", expr],
  );
}
...
function convertCallExpressionWithMemberCallee(
  ...
  return perform(
    ...
    CodeGenError,
    ["call expression with member", node],
  );
}
...
function convertExportVariableDeclaration(
  ...
  return perform(
    ...
    CodeGenError,
    ["export variable declaration", node],
  );
}
...
function convertIRExpr(node: IR.IRNode): ts.Expression {
  return perform(
    () => {
      if (!node) {
        console.warn(
          "Null node passed to convertIRExpr, returning 'undefined'",
        );
...
function createModuleVariableName(source: string): string {
  return perform(
    ...
    CodeGenError,
    ["module variable name creation", source],
  );
}
