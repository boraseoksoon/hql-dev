...
async function loadModuleByType(
  ...
  return performAsync(
    ...
    ImportError,
    [modulePath, options.currentFile],
  );
}
...
function processDefDeclaration(
  ...
  return perform(
    ...
    `Processing def declaration for ${
      isSymbol(expr.elements[1]) ? expr.elements[1].name : "unknown"
    }`,
    MacroError,
    [
      isSymbol(expr.elements[1]) ? expr.elements[1].name : "unknown",
...
function processDefnDeclaration(
  ...
  return perform(
    ...
    `Processing defn declaration for ${
      isSymbol(expr.elements[1]) ? expr.elements[1].name : "unknown"
    }`,
    MacroError,
    [
