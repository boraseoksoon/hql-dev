// deno run -A scripts/run-jsr-suite-7-8-7.ts
...
} from "jsr:@boraseoksoon/hql@7.8.7";
...
const root = Deno.cwd();
const abs = (rel: string) => path.isAbsolute(rel) ? rel : path.resolve(root, rel);
...
for (const rel of simple) {
  try {
    await runFileHql(abs(rel));
    console.log("OK ", rel);
    passed++;
  } catch (e) {
...
for (const rel of compileRun) {
  try {
    const out = await transpileCLI(abs(rel));
