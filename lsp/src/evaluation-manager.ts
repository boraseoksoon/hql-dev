...
// Create a logger instance for evaluations with verbose mode enabled
const evaluationLogger = Logger.create(true);
...
export class EvaluationManager {
  ...
  public async evaluateExpression(): Promise<void> {
    ...
    try {
      ...
      evaluationLogger.debug(`Evaluated expression: ${code} => ${result}`);
    } catch (err: any) {
      ...
      evaluationLogger.error(`Evaluation error: ${errorMessage}`);
    } finally {
      this.activeEvaluations.delete(requestId);
    }
  }
  ...
  public async evaluateOutermostExpression(): Promise<void> {
    ...
    try {
      ...
      ui.showInlineEvaluation(editor, vsCodeRange, result, code);
      evaluationLogger.debug(`Evaluated outermost expression: ${code} => ${result}`);
    } catch (err: any) {
      if (err.name === 'AbortError') {
        // Request was canceled, don't show error
        return;
  ...
  public cancelAllEvaluations(): void {
    ...
    evaluationLogger.info("All evaluations canceled");
  }
