...
function getUserErrorMessage(type: ErrorType, registry: "npm" | "jsr"): string {
  ...
  const messages = {
    ...
    [ErrorType.INSTALLATION]: {
      jsr: `${prefix}Neither jsr nor deno is installed or available. Please install jsr (https://jsr.io/cli) or deno (https://deno.com/) to publish to JSR.`,
      npm: `${prefix}npm command not found. Please ensure npm is installed and available in your PATH.`
    },
    [ErrorType.VALIDATION]: `${prefix}Package validation failed. Please check your package configuration for errors.`,
    [ErrorType.RATE_LIMIT]: `${prefix}Rate limit exceeded. Please wait a few minutes before trying again.`,
    [ErrorType.DEPENDENCY]: `${prefix}Dependency resolution failed. Please check your package dependencies and ensure they are available.`,
    [ErrorType.CONFIGURATION]: `${prefix}Configuration error detected. Please check your ${registry === "jsr" ? "jsr.json" : "package.json"} file for issues.`,
    [ErrorType.DISK_SPACE]: `${prefix}Insufficient disk space. Please free up some space and try again.`,
    [ErrorType.TIMEOUT]: `${prefix}Operation timed out. Please check your network connection and try again.`,
    [ErrorType.SERVER]: `${prefix}Server error encountered. The ${registry} registry might be experiencing issues. Please try again later.`,
    [ErrorType.UNKNOWN]: `${prefix}An unknown error occurred. Please check the log for more details.`
  };
...
function detectError(errorMessage: string, registry: "npm" | "jsr"): ErrorInfo {
  logger.debug && logger.debug(`[${registry.toUpperCase()} error] Raw error: ${errorMessage}`);
