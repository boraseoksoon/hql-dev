  ...
  if (symbols.length === 0) {
    ...
    symbols.forEach(symbol => {
      try {
        ...
        if (logger.isVerbose) {
          printError(
            `Error looking up ${symbol}: ${formatErrorMessage(error)}`,
            useColors
          );
        }
      }
  ...
  if (target === '/' || target === '*') {
    ...
    if (target === '/') {
      ...
      try {
        ...
        console.error(`Error resetting state files: ${formatErrorMessage(error)}`);
      }
      ...
      try {
        ...
        console.error(`Error syncing state: ${formatErrorMessage(error)}`);
      }
